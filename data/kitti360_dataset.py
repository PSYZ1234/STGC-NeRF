...
class KITTI360Dataset(BaseDataset):
    ...
    root_path: str = "/KITTI360"
    lidar_bin_path = "KITTI-360/data_3d_raw/2013_05_28_drive_0000_sync/velodyne_points/data"
    ...
    def __post_init__(self):
        ...
        for f in tqdm.tqdm(frames, desc=f"Loading {self.split} data"):
            ...
            # b_lidar_path = os.path.join(self.root_path, self.lidar_bin_path, 
            #                             "%010d.bin" % f['frame_id'])
            ...
            # self.poses_lidar_original.append(pose_lidar)
            self.images_lidar.append(image_lidar)
            # self.bins_lidar.append(bin_lidar)
        ...
        # self.bins_lidar = torch.from_numpy(np.stack(self.bins_lidar, axis=0)).float()  # [N, M, C]
        ...
        if self.preload:
            ...
            # self.poses_lidar_original = self.poses_lidar_original.to(self.device)
            if self.fp16:
                dtype = torch.half
            else:
                dtype = torch.float
            self.images_lidar = self.images_lidar.to(dtype).to(self.device)
            # self.bins_lidar = self.bins_lidar.to(dtype).to(self.device)
    ...
    def collate(self, index):
        ...
        # poses_lidar_original = self.poses_lidar_original[index].to(self.device)  # [B, 4, 4]
