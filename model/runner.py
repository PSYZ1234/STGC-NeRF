...
from flow.sceneflow_loss import sceneflow_loss_func
from utils.chamfer3D.dist_chamfer_3D import chamfer_3DDist
from utils.convert import pano_to_lidar, nearest_pt, compute_features, NetVLADLoupe
...
class Trainer(object):
    def __init__(
        ...
        # our model
        model_flow.to(self.device)
        self.model_flow = model_flow
        self.net_vlad = NetVLADLoupe(feature_size=128, max_samples=self.opt.num_rays_lidar, 
                                     cluster_size=64, output_dim=128)
        self.net_vlad.to(self.device) 
    ...
    def train_step(self, data):
        ...
        for param in self.net_vlad.parameters():
            param.requires_grad = False

        flow_time = self.model.flow(pred_lidar.squeeze(0) * self.opt.scale, time_lidar)
        flow_time_pos = flow_time["forward"]
        flow_time_neg = flow_time["backward"]

        with torch.cuda.amp.autocast(enabled=False):
            ### static feat
            loss_fn_g = nn.L1Loss()
            loss_fn_d = nn.MSELoss()
            pred_lidar_feat   = compute_features(pred_lidar, self.opt.knn_pt, 
                                                 self.opt.patch_size_lidar, self.opt.num_rays_lidar)
            gt_lidar_feat     = compute_features(gt_lidar, self.opt.knn_pt, 
                                                 self.opt.patch_size_lidar, self.opt.num_rays_lidar)
            results_dict_feat = self.model_flow(pred_lidar, gt_lidar)
            local_feat_preds  = results_dict_feat['feat_preds'][-1].permute(0, 2, 1)
            local_feat_gts    = results_dict_feat['feat_gts'][-1].permute(0, 2, 1)
            global_feat_preds = self.net_vlad(local_feat_preds)
            global_feat_gts   = self.net_vlad(local_feat_gts)
            if self.opt.feat_g_loss:
                loss_feat_g = loss_fn_g(pred_lidar_feat, gt_lidar_feat)
                loss = loss + self.opt.feat_g_scale * loss_feat_g # L1=5, L2=1500
            if self.opt.feat_d_loss:
                loss_lf = loss_fn_d(local_feat_preds, local_feat_gts)
                loss_gf = loss_fn_d(global_feat_preds, global_feat_gts)
                loss = loss + self.opt.feat_d_scale * (loss_lf + loss_gf) # L1=0.5, Cos=0.1

            ### dynamic feat
            f_idx = int(time_lidar * (self.opt.num_frames - 1))
            pose = self.pose_list[f"{f_idx}"]
            pred_lidar = pred_lidar * self.opt.scale
            gt_lidar = gt_lidar * self.opt.scale    
            loss_fn_f = nn.L1Loss()   
            for step in [1, 2]:
                flow_consis_loss = 0
                consis_flow_loss = 0
                if f"{f_idx+step}" in self.gt_pc_list.keys():
                    gt_pos = self.gt_pc_list[f"{f_idx+step}"]
                    pose_pos = self.pose_list[f"{f_idx+step}"]
                    gt_lidar_pos = nearest_pt(gt_lidar, gt_pos * self.opt.scale, pose, pose_pos)
                    # compute flow
                    results_dict_pred = self.model_flow(pred_lidar, gt_lidar_pos)
                    flow_3d_pred      = results_dict_pred['flow_preds'][-1]
                    results_dict_gt   = self.model_flow(gt_lidar, gt_lidar_pos)
                    flow_3d_gt        = results_dict_gt['flow_preds'][-1]
                    flow_consis_loss += sceneflow_loss_func(flow_3d_pred, flow_3d_gt)
                    consis_flow_loss += loss_fn_f(flow_time_pos * step, flow_3d_gt.squeeze(0).T)
                    # flow_inverse:
                    results_dict_pred = self.model_flow(gt_lidar_pos, pred_lidar)
                    flow_3d_pred      = results_dict_pred['flow_preds'][-1]
                    results_dict_gt   = self.model_flow(gt_lidar_pos, gt_lidar)
                    flow_3d_gt        = results_dict_gt['flow_preds'][-1]
                    flow_consis_loss += sceneflow_loss_func(flow_3d_pred, flow_3d_gt)

                if f"{f_idx-step}" in self.gt_pc_list.keys():
                    gt_neg = self.gt_pc_list[f"{f_idx-step}"]
                    pose_neg = self.pose_list[f"{f_idx-step}"]
                    gt_lidar_neg = nearest_pt(gt_lidar, gt_neg * self.opt.scale, pose, pose_neg)
                    # compute flow
                    results_dict_pred = self.model_flow(gt_lidar_neg, pred_lidar)
                    flow_3d_pred      = results_dict_pred['flow_preds'][-1]
                    results_dict_gt   = self.model_flow(gt_lidar_neg, gt_lidar)
                    flow_3d_gt        = results_dict_gt['flow_preds'][-1]
                    flow_consis_loss += sceneflow_loss_func(flow_3d_pred, flow_3d_gt)
                    # V3 for consis_flow
                    # consis_flow_loss += loss_fn_f(flow_time_neg * step, flow_3d_gt.squeeze(0).T)
                    # flow_inverse:
                    results_dict_pred = self.model_flow(pred_lidar, gt_lidar_neg)
                    flow_3d_pred      = results_dict_pred['flow_preds'][-1]
                    results_dict_gt   = self.model_flow(gt_lidar, gt_lidar_neg)
                    flow_3d_gt        = results_dict_gt['flow_preds'][-1]
                    flow_consis_loss += sceneflow_loss_func(flow_3d_pred, flow_3d_gt)
                    # V4 for consis_flow
                    consis_flow_loss += loss_fn_f(flow_time_neg * step, flow_3d_gt.squeeze(0).T)

                # loss for scene flow constraints
                if self.opt.flow_consis_loss:
                    loss = loss + self.opt.flow_consis_scale * flow_consis_loss 

                if self.opt.consis_flow_loss:
                    loss = loss + self.opt.consis_flow_scale * consis_flow_loss 
        ...
        if self.opt.flow_loss:
            ...
            loss = loss + 0.0001 * (zero_flow["forward"].abs().sum() + zero_flow["backward"].abs().sum())
    ...
    def test_step(self, data, perturb=False):
        ...
        return pred_raydrop, pred_intensity, pred_depth, gt_raydrop, gt_intensity, gt_depth
    ...
    def test(self, loader, save_path=None, name=None, write_video=True, refine=True):
        ...
        with torch.no_grad():
            for i, data in enumerate(loader):
                with torch.cuda.amp.autocast(enabled=self.fp16):
                    preds_raydrop, preds_intensity, preds_depth, \
                        gts_raydrop, gts_intensity, gts_depth = self.test_step(data)
                ...
                # save ply point cloud pred
                pcd = o3d.geometry.PointCloud()
                pcd.points = o3d.utility.Vector3dVector(pred_lidar[:, :3]) 
                o3d.io.write_point_cloud(
                    os.path.join(save_path, f"test_{name}_{i+1:04d}_depth_lidar.ply"), pcd)
                
                # save ply point cloud gt
                gt_raydrop = gts_raydrop[0].detach().cpu().numpy()
                gt_raydrop = (np.where(gt_raydrop > 0.5, 1.0, 0.0)).reshape(
                    loader._data.H_lidar, loader._data.W_lidar
                )
                gt_raydrop = (gt_raydrop * 255).astype(np.uint8)

                gt_intensity = gts_intensity[0].detach().cpu().numpy()
                gt_intensity = (gt_intensity * 255).astype(np.uint8)

                gt_depth = gts_depth[0].detach().cpu().numpy()
                gt_lidar = pano_to_lidar(
                    gt_depth / self.opt.scale, loader._data.intrinsics_lidar
                )

                pcd_gt = o3d.geometry.PointCloud()
                pcd_gt.points = o3d.utility.Vector3dVector(gt_lidar[:, :3]) 
                o3d.io.write_point_cloud(
                    os.path.join(save_path, f"test_{name}_{i+1:04d}_gt_lidar.ply"), pcd_gt)
                ...
                if write_video:
                    ...
                    all_gts.append(cv2.cvtColor(cv2.applyColorMap(gt_intensity, 1), cv2.COLOR_BGR2RGB))
                    all_gts_depth.append(cv2.cvtColor(cv2.applyColorMap(gt_depth, 20), cv2.COLOR_BGR2RGB))
                else:
                    cv2.imwrite(
                    ...
                    cv2.imwrite(
                        os.path.join(save_path, f"test_{name}_{i+1:04d}_raydrop_gt.png"),
                        gt_raydrop,
                    )
                    cv2.imwrite(
                        os.path.join(
                            save_path, f"test_{name}_{i+1:04d}_intensity_gt.png"
                        ),
                        cv2.applyColorMap(gt_intensity, 1),
                    )
                    cv2.imwrite(
                        os.path.join(save_path, f"test_{name}_{i+1:04d}_depth_gt.png"),
                        cv2.applyColorMap(gt_depth, 20),
                    )
    ...
    def process_pointcloud(self, loader):
        ...
        for i, data in enumerate(loader):
            ...
            self.gt_pc_list[f"{frame_idx}"] = gt_lidar
            self.pose_list[f"{frame_idx}"] = pose
            if i % 10 == 0:
                print(f"{i+1}/{len(loader)}")
